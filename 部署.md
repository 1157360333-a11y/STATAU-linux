...
# -w 2 表示开启2个工作进程（根据CPU核数调整），-b 绑定端口
# 使用应用工厂模式
...
# 启动命令 (设置超时时间为120秒，因为回归分析可能很慢)
ExecStart=/var/www/STATAU/venv/bin/gunicorn --workers 4 --timeout 120 --bind unix:statau.sock -m 007 "app:create_app()"

[Install]
WantedBy=multi-user.target
```
*解释：这里我们改用了 `unix:statau.sock`，使用 Socket 文件通信比端口通信稍快一点点。*
...
1.  **502 Bad Gateway**:
    *   通常是 Gunicorn 没启动成功。
    *   查看日志：`sudo journalctl -u statau`
    *   检查代码里是否有缺少的包没安装。

2.  **504 Gateway Time-out**:
    *   回归分析跑太久了。
    *   增加 Nginx 的 `proxy_read_timeout` 和 Gunicorn 的 `--timeout`。

3.  **413 Request Entity Too Large**:
    *   上传的文件超过了 Nginx 限制。
    *   检查 Nginx 配置里的 `client_max_body_size`。

4.  **"追加到当前表"功能失效**:
    *   确保安装了 `flask-session`：`pip install flask-session`
    *   确保设置了环境变量：`export FLASK_ENV=production`
    *   检查 `flask_session/` 目录是否存在且有写入权限

现在，你可以尝试在浏览器输入服务器的 IP 地址，应该就能看到你的 STATAU 平台了！祝部署顺利！
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 点击没反应 | 请求被不同 Worker 处理，缓存不共享 | 使用 Flask-Session |
| 突然增加好几列 | 多次点击的请求最终回到同一 Worker | 前端防重复点击 |

**核心修改：将 `RESULT_CACHE` 改为 `session` 存储**，这样无论请求被哪个 Worker 处理，都能访问到同一份数据。